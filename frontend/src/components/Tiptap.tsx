import '../index.css' // Ensure to include Tailwind's output CSS
...
import { FaYoutube, FaStrikethrough, FaBold, FaItalic, FaCode, FaAlignCenter, FaAlignLeft, FaAlignRight, FaListUl, FaImage, FaQuoteLeft, FaAlignJustify } from "react-icons/fa";
    ...
    return (
        <div className="font-euclid font-normal z-10 sticky top-16 md:top-[61px] py-5 drop-shadow-2xl">
            <div className="button-group flex flex-wrap justify-center gap-x-1 gap-y-2 px-10 text-sm w-[80%] md:w-fit m-auto bg-black py-3 rounded-3xl">
                ...
                <button onClick={addImage} className='bg-gray-100 border rounded-xl py-1 px-2 hover:bg-violet-500 font-semibold border-none'><FaImage /></button>
                <button id="add" onClick={addYoutubeVideo} className='bg-gray-100 border rounded-xl py-1 px-2 hover:bg-violet-500 font-semibold border-none'><FaYoutube /></button>
            </div>
        </div>
...
export const TipTap = ({ content, setContent, setWords, isEditable }: { content: string, setContent: (z: string) => void, setWords: (z: number) => void, isEditable: boolean }) => {
    ...
    return (
        <EditorProvider slotBefore={isEditable ? <MenuBar /> : null} extensions={extensions} content={jsonContent} editorProps={editorProps} editable={isEditable} onUpdate={({ editor }) => {
