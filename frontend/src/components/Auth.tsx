...
import { SignupType, SigninType, signupInput, signinInput } from '@eswar-pranav-nadh/common'
...
export const Auth = ({ type }: { type: "signin" | "signup" }) => {
    ...
    return <div className="h-dvh w-full flex flex-col justify-center items-center">
        ...
        <div className="w-1/2 py-5">
            {type == "signup" && <Helper type={"text"} placeholder={"Enter your username"} label={"Username"} onChange={(e) => {
                setSignInputs(c => ({
                    ...c, name: e.target.value
                }))
            }} />}
            <Helper type={"email"} placeholder={"m@example.com"} label={"Email"} onChange={(e) => {
                setSignInputs(c => ({
                    ...c, email: e.target.value
                }))
            }} />
            <Helper type={"password"} placeholder={""} label={"Password"} onChange={(e) => {
                setSignInputs(c => ({
...
function Helper({ type, placeholder, label, onChange }: helperType) {
    return (
        <div>
            <div className="font-medium pb-1 pt-2">
                <label htmlFor={label}>{label}</label>
            </div>
            <input className="w-full rounded-xl border-2 p-2" type={type} required={true} placeholder={placeholder} id={label} onChange={onChange}></input>
        </div>
    )
}